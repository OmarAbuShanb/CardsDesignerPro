...
class CardCanvasView @JvmOverloads constructor(
    ...
    private val renderer = TemplateRenderer(context).apply {
        ...
        val q = dev.anonymous.cardsdesignerpro.app.data.model.ExportQuality.FULL
            ...
            if (cached != null) {
                ...
                if (!previewRendering && cardWidthPx > 0 && cardHeightPx > 0) {
                    ...
                    PREVIEW_EXECUTOR.execute {
                        try {
                            val q = dev.anonymous.cardsdesignerpro.app.data.model.ExportQuality.MEDIUM
    ...
    /** When false, guide lines still appear but elements move/rotate/resize freely without magnetic pull. */
    ...
    private fun processMove(x: Float, y: Float) {
        ...
        when (mode) {
            ...
            Mode.ROTATE -> {
                ...
                // Snap if near cardinal (only when snapping is enabled)
            ...
            Mode.RESIZE_W -> {
                ...
                if (el is TemplateElement.TextElement) {
                    ...
                    // ShapeElement: snap to square (only when snapping is enabled)
                    val newW = if (isSnapEnabled && kotlin.math.abs(rawW - el.height) < SHAPE_SQUARE_SNAP) {
            ...
            Mode.RESIZE_H -> {
                ...
                // Snap to square: if height ≈ width, lock them equal (only when snapping is enabled)
                val newH = if (isSnapEnabled && kotlin.math.abs(rawH - el.width) < SHAPE_SQUARE_SNAP) {
                    if (!shapeSquareSnapped) {
                        performHapticFeedback(android.view.HapticFeedbackConstants.CLOCK_TICK)
                        shapeSquareSnapped = true
                    }
    ...
    private fun computeSnap(id: String, touchX: Float, touchY: Float): Pair<Float, Float> {
        ...
        // Find nearest snap target for guide lines (always computed for visual feedback)
        ...
        // Guide lines always shown; magnetic pull only when snapping is enabled
        snapX?.let { snapLinesX.add(it) }
        snapY?.let { snapLinesY.add(it) }

        // When snapping is disabled, use un-snapped target position (free movement)
        if (!isSnapEnabled) {
            return (targetX - el.x) to (targetY - el.y)
        }
