...
import dev.anonymous.cardsdesignerpro.app.ui.editor.canvas.CardCanvasView
...
class EditorActivity : AppCompatActivity(), CardCanvasView.Listener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Restore snap state from configuration change (defaults to true)
        savedInstanceState?.let {
            val snap = it.getBoolean("snap_enabled", true)
            binding.switchSnap.isChecked = snap
            binding.cardCanvas.isSnapEnabled = snap
        }
    }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putBoolean("snap_enabled", binding.switchSnap.isChecked)
    }
    ...
    private fun setupBackSideSwitch() {
        ...
        // Credential mode segmented button
        binding.toggleCredentialMode.addOnButtonCheckedListener { _, checkedId, isChecked ->
            if (!isChecked) return@addOnButtonCheckedListener
            val mode = when (checkedId) {
                R.id.btn_mode_normal -> CredentialMode.NORMAL
                R.id.btn_mode_short -> CredentialMode.SHORT
                R.id.btn_mode_username_only -> CredentialMode.USERNAME_ONLY
                else -> return@addOnButtonCheckedListener
            }
            if (viewModel.currentTemplate.credentialMode != mode) {
                viewModel.setCredentialMode(mode)
            }
        }

        binding.btnInfoCredentialMode.setOnClickListener {
            MaterialAlertDialogBuilder(this)
                .setTitle(R.string.credential_mode_info_title)
                .setMessage(R.string.credential_mode_info_message)
                .setPositiveButton(R.string.btn_close, null)
                .show()
        }

        // Snap toggle — session-only, defaults to ON
        binding.switchSnap.setOnCheckedChangeListener { _, isChecked ->
            binding.cardCanvas.isSnapEnabled = isChecked
        }

        binding.btnInfoSnap.setOnClickListener {
            MaterialAlertDialogBuilder(this)
                .setTitle(R.string.snap_info_title)
                .setMessage(R.string.snap_info_message)
                .setPositiveButton(R.string.btn_close, null)
                .show()
        }
    ...
    private var lastRenderedCredentialMode: CredentialMode? = null
    ...
    private fun renderState(state: EditorUiState) {
        ...
        val credentialMode = template.credentialMode
        if (lastRenderedCredentialMode != credentialMode) {
            lastRenderedCredentialMode = credentialMode
            // Temporarily remove the listener to avoid feedback loop
            binding.toggleCredentialMode.clearOnButtonCheckedListeners()
            val targetBtnId = when (credentialMode) {
                CredentialMode.NORMAL -> R.id.btn_mode_normal
                CredentialMode.SHORT -> R.id.btn_mode_short
                CredentialMode.USERNAME_ONLY -> R.id.btn_mode_username_only
            }
            binding.toggleCredentialMode.check(targetBtnId)
            // Re-attach listener
            binding.toggleCredentialMode.addOnButtonCheckedListener { _, checkedId, isChecked ->
                if (!isChecked) return@addOnButtonCheckedListener
                val mode = when (checkedId) {
