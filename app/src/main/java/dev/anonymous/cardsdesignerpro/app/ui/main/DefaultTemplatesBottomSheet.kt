        ...
        val adapter = DefaultTemplateAdapter { dirName, template ->
                ...
                if (!enteredName.isNullOrEmpty()) {
                    viewModel.extractDefaultTemplate(dirName, enteredName) { newId ->
                        if (newId != null) {
                            ...
                            (activity as? MainActivity)?.snack(getString(dev.anonymous.cardsdesignerpro.app.R.string.error_extracting_template))
                        }
                    }
                }
            }
            if (childFragmentManager.findFragmentByTag(TemplateNameDialogFragment.TAG) == null) {
                TemplateNameDialogFragment.newInstance(
                    titleRes = dev.anonymous.cardsdesignerpro.app.R.string.btn_edit_default_template,
                    positiveBtnRes = dev.anonymous.cardsdesignerpro.app.R.string.btn_save,
                    initialName = getString(dev.anonymous.cardsdesignerpro.app.R.string.template_name_copy, template.name),
                    requestKey = reqKey
