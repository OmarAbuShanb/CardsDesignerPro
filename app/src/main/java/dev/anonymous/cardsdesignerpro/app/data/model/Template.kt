...
data class CardLayoutPreset(
    ...
    companion object {
        ...
        /** Returns the index of a preset matching [columns]×[rows], or -1 for custom layouts. */
        fun indexFor(columns: Int, rows: Int): Int {
            return ALL.indexOfFirst { it.columns == columns && it.rows == rows }
        }
    }
}
...
enum class FlipEdge(@param:StringRes val labelRes: Int) {
    LONG_EDGE(R.string.flip_long_edge_desc),
    SHORT_EDGE(R.string.flip_short_edge_desc);
}
 ...
 *
 * Affected by user-selected quality:
 * - Card backgrounds and regular images (capped by [maxImageDim])
 * - QR code bitmaps (capped by [maxQrDim])
 *
 * Always rendered at [FULL] quality (not user-controllable):
 * - SVG images (rasterized to bitmaps before PDF embedding)
 * - Arabic text bitmap fallback (when native PDF font encoding fails)
 *
 ...
 * @property maxQrDim Cap for QR code bitmaps.
 */
@Serializable
enum class ExportQuality(
    ...
    FULL(renderScale = 15.0f, maxImageDim = 4096, maxQrDim = 1024),
    /** High quality: default. */
    HIGH(renderScale = 12.0f, maxImageDim = 3072, maxQrDim = 768),
    /** Good quality: balanced size and clarity. */
    GOOD(renderScale = 9.0f, maxImageDim = 2048, maxQrDim = 512),
    /** Medium quality: smaller files. */
    MEDIUM(renderScale = 6.5f, maxImageDim = 1536, maxQrDim = 350),
    /** Low quality: fastest draft export. */
    LOW(renderScale = 4.0f, maxImageDim = 1024, maxQrDim = 200);
}
