...
import dev.anonymous.cardsdesignerpro.app.util.PdfBoxExporter
                ...
                when (request.mode) {
                    ExportManager.ExportRequest.Mode.SINGLE -> {
                        ctx.contentResolver.openOutputStream(request.outputUri!!)?.use { stream ->
                            PdfBoxExporter.export(ctx, request.template, request.parseResult, request.shortParseResult, request.settings, stream) { done, total ->
                    ...
                    ExportManager.ExportRequest.Mode.DUAL -> {
                        ctx.contentResolver.openOutputStream(request.outputUri!!)?.use { stream ->
                            PdfBoxExporter.exportDual(ctx, request.template, request.parseResult, request.shortParseResult, request.settings, stream) { done, total ->
                                updateProgressNotification(templateName, done, total)
                                ExportManager.exportProgress.value = done.toFloat() / total
                            }
                        }
                    ...
                    ExportManager.ExportRequest.Mode.SEPARATE -> {
                        ...
                        if (fs != null && bs != null) {
                            fs.use { f ->
                                bs.use { b ->
                                    PdfBoxExporter.exportSeparate(ctx, request.template, request.parseResult, request.shortParseResult, request.settings, f, b) { done, total ->
                                        updateProgressNotification(templateName, done, total)
                                        ExportManager.exportProgress.value = done.toFloat() / total
                                    }
                                }
